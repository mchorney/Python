...
def user_page(request, user_id):
    ...
    reviewed_books = Book.objects.annotate(review_count=Count(
        'reviews')).filter(reviews__reviewer_id=user_id)
...
def add_review(request, book_id):
    if len(request.POST['content']) > 0:
        new_review = Review(content=request.POST['content'], rating=request.POST['rating'],
                            book_id=book_id, reviewer_id=get_logged_in_user(request).id)
        new_review.save()
    else:
...
def add_review_book(request):
    ...
    if len(errors) > 0:
        ...
        new_book.save()
        new_review = Review(content=request.POST['review_content'],
                            rating=request.POST['review_rating'], book=new_book, reviewer=get_logged_in_user(request))
        new_review.save()
