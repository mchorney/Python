...
def register_action(request):
    ...
    if len(errors) > 0:
        ...
        new_user = User(first_name=request.POST['first_name'], last_name=request.POST['last_name'],
                        email=request.POST['email'], password_hash=new_hash, dob=request.POST['dob'])
        new_user.save()
...
def add_friend(request, user_id):
    if logged_in(request):
        Friendship.objects.create(
            friend_a_id=user_id, friend_b_id=get_logged_in_user(request).id)
        Friendship.objects.create(friend_a_id=get_logged_in_user(
            request).id, friend_b_id=user_id)
    return redirect('/friends')


def remove_friend(request, user_id):
    if logged_in(request):
        friendship_to_remove_a = Friendship.objects.get(
            friend_a_id=user_id, friend_b_id=get_logged_in_user(request).id)
        friendship_to_remove_a.delete()
        friendship_to_remove_b = Friendship.objects.get(
            friend_a_id=get_logged_in_user(request).id, friend_b_id=user_id)
